...
_DISCLAIMER = (
    "这张卡只帮家人把看法聊清楚、把用钱计划对齐，不评判谁对谁错，"
    "也不构成投资建议。家里真正要用钱的计划永远排第一。"
)
...
def build_family_dialogue(
    ...
    try:
        ...
        if trigger == "gap":
            opening = (
                f"这次发现，家里记下的想法和现在的持仓在「{topics_text}」上有点对不上。"
                "很常见——先别急着论对错，一起看看哪里需要对齐。"
            )
        else:
            opening = (
                f"这次发现家里在「{topics_text}」上看法不太一样。很正常，也说明大家都在"
                "认真为这个家考虑。先别急着争对错，一起把它聊清楚。"
            )
        ...
        closing = (
            "聊到彼此都清楚对方在担心什么、对“下一步先看什么”有个一致方向，就算聊出结果了。"
            "不用今天就做决定，先把想法对齐最重要。"
        )
